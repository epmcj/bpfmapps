...
int recv_table_list_request(void *buffer, Header *header) {
    ...
    if (ret == -1) {
        ...
        if (tab_entry->type == BPF_MAP_TYPE_HASH) {
            ...
            item_size = tab_entry->key_size;
            /* (2 * max entries) is a loose upper bound */
            items = calloc(2 * tab_entry->max_entries, item_size);

            unsigned char *key = items;
            unsigned char *last_key = NULL;

            while (bpf_get_next_key(tab_entry->fd, key, last_key) == 0) {
                n_items++;
                last_key = key;
                key = key + item_size;
            }
