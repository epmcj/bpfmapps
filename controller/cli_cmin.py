...
class eBPFCLIApplication(eBPFCoreApplication):
    ...
    def table_list_reply(self, connection, pkt):
        ...
        if pkt.HasField('items') and pkt.HasField('entry'):
            if pkt.entry.table_name == "firsts":
		print "founf firsts"
                item_size = pkt.entry.value_size

                for i in range(pkt.n_items):
                    addr1, addr2, sec, nsec = struct.unpack_from('IIII', pkt.items, i * item_size)
		    key = Int2IP(addr1) + ":" + Int2IP(addr2)
                    entries.append((key, sec, nsec))
                    
                tabulate(entries, headers=['Pair', 'Seconds', 'Nanoseconds'])
