...
class eBPFCLIApplication(eBPFCoreApplication):
    ...
    def table_list_reply(self, connection, pkt):
        ...
        if pkt.HasField('items') and pkt.HasField('entry'):
            if pkt.entry.table_name == PROTO_TABLE_NAME:
                item_size = pkt.entry.value_size
                fmt = "{}s".format(pkt.entry.value_size)

                for i in range(pkt.n_items):
                    np, nb = struct.unpack_from('IQ', pkt.items, i * item_size)
                    entries.append((pname[i], np, nb))
                    
                tabulate(entries, headers=['Protocol', 'Num Pckts', 'Num bytes'])
